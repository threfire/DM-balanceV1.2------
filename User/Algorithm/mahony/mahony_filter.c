...
//    mahony_filter->gyro.z *= DEG2RAD;;
		...
		//减去零飘
    mahony_filter->gyro.z -= mahony_filter->gyro_z_bias;   
...
void mahony_init(struct MAHONY_FILTER_t *mahony_filter,float Kp,float Ki,float dt)
{
    ...
    mahony_filter->RotationMatrix_update = RotationMatrix_update;
		mahony_filter->gyro_z_bias = 0.0f;   
}

/* 上电静止时调用，采样 1000 次取平均，计算 z 轴零偏 */
void mahony_calibrate_gyro_bias(struct MAHONY_FILTER_t *mahony_filter,
                                float get_gyro,   /* 传入你的陀螺仪读数函数 */
                                uint32_t samples)
{
    static float sum = 0.0f;
    sum += get_gyro;
    mahony_filter->gyro_z_bias = sum / samples;
}
